...
i2s_bits_per_sample_t Speaker_Class::_bits_per_sample = I2S_BITS_PER_SAMPLE_8BIT;  // init

esp_err_t
Speaker_Class::_setup_i2s(void) {
  ...
  i2s_config = {
      ...
      .sample_rate     = 48000,
      .bits_per_sample = _bits_per_sample = I2S_BITS_PER_SAMPLE_32BIT,  // add @riraosan
      ...
      .dma_buf_count                      = (int)_cfg.dma_buf_count,
      .dma_buf_len                        = (int16_t)_cfg.dma_buf_len,
  ...
  if (_cfg.use_dac) {
    ...
    if (_cfg.i2s_port == I2S_NUM_0) {  /// レジスタを操作してDACモードの設定を有効にする(I2S0のみ。I2S1はDAC,ADC非対応) ;
...
static void calcClockDiv(uint32_t* div_a, uint32_t* div_b, uint32_t* div_n, uint32_t baseClock, uint32_t targetFreq) {
  if (baseClock <= targetFreq << 1) {  /// Nは最小2のため、基準クロックが目標クロックの2倍より小さい場合は値を確定する;
    *div_n = 2;
    *div_a = 1;
    *div_b = 0;
    return;
  ...
  if (targetFreq) {
    ...
    if (n < 256) {
      ...
      if (n < 255) {  /// 初期値の設定はNがひとつ上のものを設定しておく;
        save_a    = 1;
        save_b    = 0;
        save_n    = n + 1;
...
void Speaker_Class::spk_task(void* args) {
  ...
  const int32_t spk_sample_rate_x256 = 128000 * (256 / 2);
  ...
  const float  magnification   = (float)self->_cfg.magnification / spk_sample_rate_x256 / (~0u >> ((self->_cfg.use_dac || self->_cfg.buzzer) ? 0 : 8));
  ...
  while (self->_task_running) {
    ...
    if (!flg_i2s_started) {
      ...
      if (self->_cfg.use_dac && self->_cfg.dac_zero_level != 0) {
        ...
        size_t write_bytes;
        if (_bits_per_sample == I2S_BITS_PER_SAMPLE_16BIT) {
    ...
    for (size_t ch = 0; ch < sound_channel_max; ++ch) {
      ...
      label_next_wav:
        bool clear_idx = (next_wav->repeat == 0 || !next_wav->no_clear_index || (next_wav->data != current_wav->data));
        current_wav->clear();
    ...
    if (_bits_per_sample == I2S_BITS_PER_SAMPLE_16BIT) {
      ...
      if (_bits_per_sample > I2S_BITS_PER_SAMPLE_16BIT) {
        // expand e.g to 32 bit for DACs which do not support 16 bits
        if (i2s_write_expand(i2s_port, (void*)sound_buf, dma_buf_len * sizeof(int16_t) << self->_cfg.buzzer, I2S_BITS_PER_SAMPLE_16BIT, _bits_per_sample, &write_bytes, portMAX_DELAY) != ESP_OK) {
          log_e("i2s_write has failed");
        }
      } else {
        log_e("invalid bits_per_sample: %d", I2S_BITS_PER_SAMPLE_32BIT);
...
bool Speaker_Class::playWav(const uint8_t* wav_data, size_t data_len, uint32_t repeat, int channel, bool stop_current_sound) {
  ...
  // */
  if (memcmp(wav->RIFF, "RIFF", 4) || memcmp(wav->WAVEfmt, "WAVEfmt ", 8) || memcmp(wav->data, "data", 4) || wav->audiofmt != 1 || wav->bit_per_sample < 8 || wav->bit_per_sample > 16 || wav->channel == 0 || wav->channel > 2) {
    return false;
  }

  data_len = data_len > sizeof(wav_header_t) ? data_len - sizeof(wav_header_t) : 0;
  if (data_len > wav->wav_size) {
    data_len = wav->wav_size;
  }
  bool flg_16bit = (wav->bit_per_sample >> 4);
  return _play_raw(&wav_data[sizeof(wav_header_t)], data_len >> flg_16bit, flg_16bit, flg_16bit, wav->sample_rate, wav->channel > 1, repeat, channel, stop_current_sound, false);
}
