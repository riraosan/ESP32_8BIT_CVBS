...
#define WebPrintf(c, fmt, ...)                                      \
  {                                                                 \
    char webBuff[192];                                              \
    snprintf_P(webBuff, sizeof(webBuff), PSTR(fmt), ##__VA_ARGS__); \
    (c)->print(webBuff);                                            \
    delay(10);                                                      \
  }
#define WebPrintfPSTR(c, fmt, ...)                              \
  {                                                             \
    char webBuff[192];                                          \
    snprintf_P(webBuff, sizeof(webBuff), (fmt), ##__VA_ARGS__); \
    (c)->print(webBuff);                                        \
    delay(10);                                                  \
  }
...
#define ParamText(name, dest)                                            \
  {                                                                      \
    if (!strcmp(namePtr, (name))) strlcpy((dest), valPtr, sizeof(dest)); \
  }
#define ParamCheckbox(name, dest)                                 \
  {                                                               \
    if (!strcmp(namePtr, (name))) (dest) = !strcmp("on", valPtr); \
  }
#define ParamInt(name, dest)                               \
  {                                                        \
    if (!strcmp(namePtr, (name))) ParseInt(valPtr, &dest); \
  }
#define Param4Int(name, dest)                               \
  {                                                         \
    if (!strcmp(namePtr, (name))) Read4Int(valPtr, (dest)); \
  }
